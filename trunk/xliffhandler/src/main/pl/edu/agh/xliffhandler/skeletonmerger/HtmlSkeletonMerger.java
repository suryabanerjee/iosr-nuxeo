 ...
 * 02110-1301, USA
 */
package pl.edu.agh.xliffhandler.skeletonmerger;
...
//            Matcher am = Pattern.compile("\\b" + attName + "=(['\"])(.*?)(\\1)",
        ...
        if (parentTagStart != -1) {
            ...
            if (parentTagEnd > 0 && skelBuf.charAt(parentTagEnd -1) == '/') {
                ...
                while (!parentTagEndFound) {
                    // Check to see if this '>' ends a placeholder:
                    if (placeHolderMatcher.reset(skelBuf.subSequence(parentTagStart,parentTagEnd+1)).find()) {
                        // We found a placeholder (because the subsequence that
                        // begins with parentTagStart and ends with ParentTagEnd
                        // includes a subsequence like "<lt:tu id..../>").
                        // Question: Is the end of the place holder at the same
                        // position as parentEndTag?
                        int endPlaceHolder = placeHolderMatcher.end() + parentTagStart - 1;
                        if (endPlaceHolder == parentTagEnd) {
                            // The last '>' of the placeholder is at the same
                            // index as '>' that we (previously) thought was the
                            // end of the parent tag. ... But we were wrong; 
                            // let's look further.
                            parentTagEnd = skelBuf.indexOf(">", endPlaceHolder+1);
                            ...
                            // I s'pose we can assume that the '>' ends the
                            // parent tag.
                            parentTagEndFound = true;  // Let's stop looking
                        }
                    }
                    else {
                        // We didn't find a placeholder at all. ... So we must
                        // must've found the end of the parent tag ...
